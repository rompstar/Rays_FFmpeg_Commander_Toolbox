# ffmpeg_commander.py — v0.9.15
# - Target resolution now behaves as a minimum (only upscale; never downscale).  
# - Updated help text and label accordingly.  
# - Upscaling toggle still applies only when Target = Original (SD→720p).  
...
def load_style():  
    ...
    @define-color sol8_light     #EEF3FA;  
    @define-color sol8_shadow    #5C6C86;  
    @define-color sol8_ink       #0B2239;  
    @define-color sol8_fill      #F7F9FC;  
    @define-color sol8_progress  #6B8FC7;  
  
    window, GtkWindow {  
        background-color: @sol8_bg;  
        color: @sol8_ink;  
    ...
    button {  
        background-color: @sol8_panel;  
        color: @sol8_ink;  
        border-width: 2px;  
        border-style: solid;  
        border-top-color:   @sol8_light;  
        border-left-color:  @sol8_light;  
        border-bottom-color:@sol8_shadow;  
        border-right-color: @sol8_shadow;  
        border-radius: 0;  
        padding: 3px 10px;  
        box-shadow: none;  
    }  
    button:active {  
        background-color: shade(@sol8_panel, 0.9);  
        border-top-color:   @sol8_shadow;  
        border-left-color:  @sol8_shadow;  
        border-bottom-color:@sol8_light;  
        border-right-color: @sol8_light;  
    }  
    button:disabled {  
        color: mix(@sol8_ink, @sol8_panel, 0.4);  
    }  
    entry, combobox, spinbutton, treeview, textview, scrolledwindow {  
        background-color: @sol8_fill;  
        color: @sol8_ink;  
        border-width: 2px;  
        border-style: solid;  
        border-top-color:   @sol8_shadow;  
    ...
    textview {  
        background: @sol8_fill;  
        font-family: 'Lucida Sans Typewriter', 'DejaVu Sans Mono', monospace;  
        font-size: 10pt;  
    }  
    progressbar {  
        background: @sol8_panel;  
        border-width: 2px;  
        border-style: solid;  
        border-top-color:   @sol8_light;  
        border-left-color:  @sol8_light;  
    ...
    progressbar > trough { background: @sol8_fill; padding: 0; margin: 0; min-height: 40px; border: none; }  
    progressbar > trough > progress { background-image: none; background-color: @sol8_progress; border: none; margin: 0; min-height: 40px; }  
    #donate-button { background-image: none; background-color: #cf4f7a; color: white; font-weight: bold;  
        border-top-color:@sol8_light; border-left-color:@sol8_light; border-bottom-color:@sol8_shadow; border-right-color:@sol8_shadow; }  
    #donate-button:hover { background-color: #dd5c88; }  
    """  
    sp = Gtk.CssProvider()  
    sp.load_from_data(css.encode("utf-8"))  
...
class FFmpegGUI(Gtk.Window):  
    ...
    def __init__(self):  
        ...
        # Row 1: Container + Video encoder  
        row1 = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)  
        vbox.pack_start(row1, False, False, 0)  
  
        col_container = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)  
        row1.pack_start(col_container, True, True, 0)  
        col_container.pack_start(Gtk.Label(label="Container (mp4, mkv, mov, avi, webm).", xalign=0), False, False, 0)  
        ...
        self.combo_format.pack_start(r_fmt, True)  
        self.combo_format.add_attribute(r_fmt, "text", 0)  
        self.combo_format.set_active(0)  
        self.combo_format.set_hexpand(True)  
        self.combo_format.connect("changed", self.on_format_changed)  
        col_container.pack_start(self.combo_format, False, False, 0)  
  
        col_encoder = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)  
        row1.pack_start(col_encoder, True, True, 0)  
        col_encoder.pack_start(Gtk.Label(label="Video encoder", xalign=0), False, False, 0)  
        ...
        self.combo_codec.pack_start(r_codec, True)  
        self.combo_codec.add_attribute(r_codec, "text", 0)  
        self.combo_codec.set_active(0)  
        self.combo_codec.set_hexpand(True)  
        self.combo_codec.connect("changed", self.on_codec_changed)  
        col_encoder.pack_start(self.combo_codec, False, False, 0)  
  
        # Row 2: Preset + Rate control  
        row2 = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=12)  
        vbox.pack_start(row2, False, False, 0)  
  
        col_preset = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)  
        row2.pack_start(col_preset, True, True, 0)  
        col_preset.pack_start(  
            Gtk.Label(label="Preset (speed/quality trade‑off; x264/x265 and NVENC).", xalign=0), False, False, 0  
        )  
        ...
        self.combo_preset.pack_start(r_preset, True)  
        self.combo_preset.add_attribute(r_preset, "text", 0)  
        self.combo_preset.set_active(5)  
        self.combo_preset.set_hexpand(True)  
        self.combo_preset.connect("changed", self.on_params_changed)  
        col_preset.pack_start(self.combo_preset, False, False, 0)  
  
        col_rc = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)  
        row2.pack_start(col_rc, True, True, 0)  
        col_rc.pack_start(Gtk.Label(label="Rate control (NVENC): VBR or CBR.", xalign=0), False, False, 0)  
        ...
        self.combo_rc.pack_start(r_rc, True)  
        self.combo_rc.add_attribute(r_rc, "text", 0)  
        self.combo_rc.set_active(0)  
        self.combo_rc.set_hexpand(True)  
        self.combo_rc.connect("changed", self.on_params_changed)  
        col_rc.pack_start(self.combo_rc, False, False, 0)  
        ...
        self.scale_cq.connect("value-changed", self.on_params_changed)  
        ...
        self.entry_bitrate.connect("changed", self.on_params_changed)  
        ...
        self.chk_spatial.connect("toggled", self.on_params_changed)  
        ...
        self.chk_bw.connect("toggled", self.on_params_changed)  
        vbox.pack_start(self.chk_bw, False, False, 0)  
  
        # Target resolution dropdown (minimum behavior)  
        target_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)  
        vbox.pack_start(target_box, False, False, 0)  
        target_box.pack_start(  
            Gtk.Label(label="Target resolution (minimum — only upscale; never downscale)", xalign=0),  
            False, False, 0  
        )  
        self.store_target = Gtk.ListStore(str, int)  
        for lbl, h in [  
            ("Original (keep size)", 0),  
            ("720p", 720),  
            ("1080p", 1080),  
            ("1440p", 1440),  
            ("2160p / 4K", 2160),  
        ]:  
            self.store_target.append([lbl, h])  
        self.combo_target = Gtk.ComboBox.new_with_model(self.store_target)  
        r_t = Gtk.CellRendererText()  
        self.combo_target.pack_start(r_t, True)  
        self.combo_target.add_attribute(r_t, "text", 0)  
        self.combo_target.set_active(0)  
        self.combo_target.set_hexpand(True)  
        self.combo_target.connect("changed", self.on_target_res_changed)  
        target_box.pack_start(self.combo_target, False, False, 0)  
  
        # Upscale toggle + help  
        self.chk_upscale = Gtk.CheckButton(  
            label="Allow Upscaling (Original target only: auto-upscale SD sources to 720p)"  
        )  
        self.chk_upscale.set_active(False)  
        self.chk_upscale.connect("toggled", self.on_params_changed)  
        vbox.pack_start(self.chk_upscale, False, False, 0)  
  
        self.label_upscale_help = Gtk.Label(xalign=0, wrap=True, label="")  
        vbox.pack_start(self.label_upscale_help, False, False, 0)  
        ...
        self.combo_audio_codec.pack_start(r_aud, True)  
        self.combo_audio_codec.add_attribute(r_aud, "text", 0)  
        self.combo_audio_codec.set_active(0)  
        self.combo_audio_codec.set_hexpand(True)  
        self.combo_audio_codec.connect("changed", self.on_params_changed)  
    ...
    def on_target_res_changed(self, combo):  
        target_h = self.get_target_height()  
        # Disable toggle when a specific target is chosen; enable for Original  
        self.chk_upscale.set_sensitive(target_h == 0)  
        self.update_upscale_help_text()  
  
    def update_upscale_help_text(self):  
        target_h = self.get_target_height()  
        if target_h == 0:  
            msg = ("Upscaling doesn’t create new detail. With Target = Original, enabling “Allow Upscaling” will upscale "  
                   "SD sources (<720p) to 720p for consistency; HD/4K keep their original size. Even dimensions are enforced.")  
        else:  
            msg = ("Upscaling doesn’t create new detail. With a Target resolution selected, the app only upscales smaller "  
                   f"videos up to {target_h}p; larger videos keep their original resolution (no downscaling). Aspect ratio is "  
                   "preserved and even dimensions are enforced. The Upscaling toggle is disabled because the target acts as a minimum.")  
        self.label_upscale_help.set_text(msg)  
    ...
    def on_choose_folders(self, widget):  
        dialog = Gtk.FileChooserDialog(  
            title="Select Folders", parent=self, action=Gtk.FileChooserAction.SELECT_FOLDER  
        )  
    ...
    def get_input_bit_depth(self, path):  
        try:  
            j = subprocess.check_output(  
                ["ffprobe","-v","error","-select_streams","v:0","-show_entries","stream=bits_per_raw_sample,pix_fmt","-of","json",path],  
                text=True,  
            )  
    ...
    def get_probe_target(self, path):  
        if self.is_videots(path):  
            ts = path if os.path.basename(path).upper() == "VIDEO_TS" else os.path.join(path, "VIDEO_TS")  
            vobs = self.get_videos(ts if os.path.isdir(ts) else path)  
            if vobs:  
                return vobs[0]  
        return path  
    ...
    def probe_stream(self, path):  
        try:  
            j = subprocess.check_output(  
                ["ffprobe","-v","error","-select_streams","v:0","-show_entries","stream=codec_name,bit_rate,width,height,pix_fmt,avg_frame_rate","-of","json",path],  
                text=True,  
            )  
            s = json.loads(j)["streams"][0]  
            return {"codec": s.get("codec_name"),"bit_rate": int(s.get("bit_rate") or 0),"w": int(s.get("width") or 0),  
                    "h": int(s.get("height") or 0),"pix_fmt": s.get("pix_fmt") or "","avg_frame_rate": s.get("avg_frame_rate") or ""}  
        except Exception:  
            return {"codec": None,"bit_rate": 0,"w": 0,"h": 0,"pix_fmt": "","avg_frame_rate": ""}  
    ...
    def _detect_gpu(self, sync_label_update=False):  
        ...
        names_str = " | ".join(gpus)  
        self._legacy_nvenc = any(  
            re.search(p, names_str, re.I)  
            for p in [r"GTX\s*10", r"GTX\s*9", r"GTX\s*7", r"\bQuadro\s*M", r"\bQuadro\s*P", r"\bTesla\s*[KM]", r"TITAN\s*X"]  
        )  
  
        caps_txt = []  
        for enc, caps in self._nvenc_caps.items():  
            flags = []  
            if caps.get("temporal_aq"): flags.append("tempAQ")  
            if caps.get("rc_lookahead"): flags.append("lookahead")  
            if caps.get("p010"): flags.append("p010")  
            if flags:  
                caps_txt.append(f"{enc}: {'/'.join(flags)}")  
        caps_line = f" | Caps: {'; '.join(caps_txt)}" if caps_txt else ""  
  
        nvdec_line = (f"NVENC: {nvenc_label}"  
                      + (", CUDA hwaccel" if self._have_cuda_hwaccel else "")  
                      + f" | NVDEC/cuvid: {','.join(sorted(self._nvdec_codecs)) or 'none'}"  
                      + f" | CUDA filters: {'yes' if self._have_hwupload_cuda else 'no'}"  
                      + (" | legacy-safe" if self._legacy_nvenc else ""))  
        label_text = f"GPUs: {', '.join(gpus)}\n{nvdec_line}{caps_line}"  
    ...
    def _query_nvenc_help(self, codec):  
        ...
        try:  
            out = subprocess.check_output(["ffmpeg","-hide_banner","-h",f"encoder={codec}"], text=True, stderr=subprocess.STDOUT)  
    ...
    def get_target_height(self):  
        try:  
            it = self.combo_target.get_active_iter()  
            if it is None:  
                return 0  
            return int(self.combo_target.get_model()[it][1])  
        except Exception:  
            return 0  
  
    def compute_scale_expr(self, src_path=None, src_info=None):  
        # Returns (expr, prefer_gpu). expr like "-2:1080" or "1280:720". None if no scaling needed.  
        allow_up = self.chk_upscale.get_active() if hasattr(self, "chk_upscale") else False  
        target_h = self.get_target_height()  
        try:  
            info = src_info if src_info is not None else (self.probe_stream(self.get_probe_target(src_path)) if src_path else {})  
        ...
        # Minimum behavior: only upscale if source is smaller; never downscale  
        if target_h > 0:  
            if h < target_h:  
                return (f"-2:{target_h}", True)  
  
        # Target = Original (keep size). Optionally upscale SD → 720p.  
        elif allow_up and h < 720:  
            return ("-2:720", True)  
  
        # No scale; just enforce even dims if needed  
        w_even = w - (w % 2)  
        h_even = h - (h % 2)  
        if w_even != w or h_even != h:  
            if w_even == 0: w_even = 2  
            if h_even == 0: h_even = 2  
            return (f"{w_even}:{h_even}", True)  
    ...
    def choose_super_settings(self, info):  
        ...
        if use_hevc_nvenc or use_h264_nvenc:  
            ...
            want_p010 = ((not self._legacy_nvenc)  
                         and self._nvenc_caps.get("hevc_nvenc", {}).get("p010", False)  
                         and re.search(r"(p010|p10|10)", info.get("pix_fmt", "")))  
    ...
    def on_super_convert(self, _):  
        ...
        probe_target = self.get_probe_target(first)  
        ...
        while itp:  
            if self.store_preset[itp][0] == rec["preset"]:  
                self.combo_preset.set_active_iter(itp)  
                break  
    ...
    def update_overall_eta(self, elapsed_in_current, speed):  
        if self.total_content_seconds > 0:  
            ...
            part = (min(1.0, elapsed_in_current / self.current_item_duration)  
                    if self.current_item_duration > 0 else 0.0)  
    ...
    def process_batch(self):  
        ...
        for idx, path in enumerate(self.batch_files):  
            ...
            self.current_item_duration = (  
                self.item_durations[idx] if idx < len(self.item_durations) else self.compute_item_duration(path)  
            )  
            ...
            out_file_name = (  
                f"{base}_converted_{idx + 1}.{ext}"  
                if len(self.batch_files) > 1  
                else (self.entry_output.get_text() or f"{base}_converted.{ext}")  
            )  
            ...
            if self.is_videots(work_path):  
                ...
                info_first = self.probe_stream(vobs[0])  
                scale_expr, _ = self.compute_scale_expr(src_info=info_first)  
  
                streams = "".join(f"[{i}:v:0][{i}:a:0]" for i in range(n))  
                base_fc = f"{streams}concat=n={n}:v=1:a=1[v][a]"  
                post_filters = []  
                if self.chk_bw.get_active():  
                    post_filters.append("hue=s=0")  
                if scale_expr:  
                    post_filters.append(f"scale={scale_expr}")  
  
                if post_filters:  
                    filter_complex = f"{base_fc};[v]{','.join(post_filters)}[vout]"  
                    vmap = "[vout]"  
                else:  
                ...
                video_args = self.build_video_args(  
                    codec, preset, rc_mode, cq, bitrate_value, bit_depth, self.chk_spatial.get_active(),  
                    ext, using_cuda_frames=False, nvenc_safe=False,  
                )  
                cmd = ["ffmpeg","-y",*input_args,"-filter_complex",filter_complex,"-map",vmap,"-map","[a]",  
                       *video_args,*tag_args,"-c:a",audio_codec,"-b:a","192k",*movflags_args,"-f",ext,out_file]  
                ...
                if not success and codec in ("h264_nvenc", "hevc_nvenc"):  
                    ...
                    video_args = self.build_video_args(  
                        codec, preset, rc_mode, cq, bitrate_value, bit_depth, self.chk_spatial.get_active(),  
                        ext, using_cuda_frames=False, nvenc_safe=True,  
                    )  
                    cmd = ["ffmpeg","-y",*input_args,"-filter_complex",filter_complex,"-map",vmap,"-map","[a]",  
                           *video_args,*tag_args,"-c:a",audio_codec,"-b:a","192k",*movflags_args,"-f",ext,out_file]  
                    ret, stderr_text = self.run_ffmpeg_sync(cmd, vobs[0], duration_override=self.current_item_duration)  
                ...
                info_src = self.probe_stream(original_path)  
                scale_expr, prefer_gpu = self.compute_scale_expr(src_info=info_src)  
                filters = []  
                require_cpu_filters = False  
                if self.chk_bw.get_active():  
                    filters.append("hue=s=0")  
                    require_cpu_filters = True  
                if scale_expr:  
                    if (prefer_gpu and not require_cpu_filters  
                        and codec in ("h264_nvenc","hevc_nvenc") and self._have_cuda_hwaccel and self._have_hwupload_cuda):  
                        filters.append(f"scale_cuda={scale_expr}")  
                    else:  
                        filters.append(f"scale={scale_expr}")  
                        require_cpu_filters = True  
                vf = ",".join(filters) if filters else None  
  
                can_hw_frames = (  
                    (codec in ("h264_nvenc", "hevc_nvenc"))  
                    and self._have_cuda_hwaccel and self._have_hwupload_cuda and (not require_cpu_filters)  
                )  
                input_prefix = []  
                forced_decoder = None  
                if can_hw_frames:  
                    try:  
                        probe = subprocess.check_output(  
                            ["ffprobe","-v","error","-select_streams","v:0","-show_entries","stream=codec_name","-of","default=nw=1:nk=1",original_path],  
                            text=True,  
                        ).strip()  
                    except Exception:  
                        probe = ""  
                    ...
                    if forced_decoder:  
                        ...
                        can_hw_frames = False  
                        if vf and "scale_cuda=" in vf:  
                            vf = vf.replace("scale_cuda=", "scale=")  
                            require_cpu_filters = True  
                ...
                video_args = self.build_video_args(  
                    codec, preset, rc_mode, cq, bitrate_value, bit_depth, self.chk_spatial.get_active(),  
                    ext, using_cuda_frames=can_hw_frames, nvenc_safe=False,  
                )  
                ...
                if (not success and can_hw_frames and (  
                    "auto_scale_0" in stderr_text or "Function not implemented" in stderr_text  
                    or "Impossible to convert between the formats" in stderr_text)):  
                    ...
                    if vf:  
                        vf2 = vf.replace("scale_cuda=", "scale=")  
                        cmd += ["-vf", vf2]  
                    cmd += ["-c:a",audio_codec,"-b:a","192k",*movflags_args,"-f",ext,out_file]  
                    ret, stderr_text = self.run_ffmpeg_sync(cmd, original_path, duration_override=self.current_item_duration)  
                    success = ret == 0  
  
                if (not success and codec in ("h264_nvenc","hevc_nvenc") and (  
                    "Temporal AQ not supported" in stderr_text or "rc-lookahead" in stderr_text  
                    or "Provided device doesn't support required NVENC features" in stderr_text  
                    or "Could not open encoder" in stderr_text)):  
                ...
                if not success and codec == "hevc_nvenc" and "h264_nvenc" in self._nvenc_available:  
                    ...
                    codec2 = "h264_nvenc"  
                    video_args = self.build_video_args(  
                        codec2, preset, rc_mode, cq, bitrate_value, 8, self.chk_spatial.get_active(),  
                        ext, using_cuda_frames=False, nvenc_safe=True,  
                    )  
                ...
                if not success and codec in ("h264_nvenc","hevc_nvenc"):  
                    ...
                    video_args = self.build_video_args(  
                        codec3, preset3, "crf", cq3, bitrate_value, 8, False, ext, using_cuda_frames=False,  
                    )  
                    cmd = ["ffmpeg","-y","-i",original_path,*video_args]  
                    if vf:  
                        vf2 = vf.replace("scale_cuda=", "scale=")  
    ...
    def build_video_args(self, codec, preset, rc_mode, cq, bitrate_kbps, desired_bit_depth,  
        ...
        if codec in ("h264_nvenc", "hevc_nvenc"):  
            nvenc_preset_map = {"ultrafast":"p1","superfast":"p1","veryfast":"p2","faster":"p3","fast":"p4",  
                                "medium":"p5","slow":"p6","slower":"p6","veryslow":"p7","placebo":"p7"}  
            ...
            if not safe:  
                if caps.get("temporal_aq", False): args += ["-temporal-aq","1"]  
                if caps.get("rc_lookahead", False): args += ["-rc-lookahead","20"]  
            ...
            if codec == "libvpx-vp9":  
                ...
                if bitrate_kbps and bitrate_kbps > 0:  
                    ...
                    args += ["-b:v","0","-crf",str(int(cq))]  
                args += ["-row-mt","1","-tile-columns","2","-frame-parallel","1","-deadline","good","-cpu-used","2"]  
            elif codec == "libvpx":  
                args += ["-pix_fmt","yuv420p"]  
                if bitrate_kbps and bitrate_kbps > 0:  
                    args += ["-b:v", f"{bitrate_kbps}k"]  
    ...
    def run_ffmpeg_sync(self, cmd, input_file, duration_override=None):  
        ...
        try:  
            ...
            for line in iter(process.stderr.readline, ""):  
                ...
                if mt:  
                    try:  
                        h = int(mt.group(1)); m_ = int(mt.group(2)); s = int(mt.group(3)); frac = float("0." + mt.group(4))  
