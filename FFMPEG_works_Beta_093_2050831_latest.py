# ffmpeg_commander.py — v0.9.12  
# - Compact UI: Container+Encoder on one row; Preset+Rate Control on one row.  
# - Robust NVENC/NVDEC handling and Super Convert behavior.  
# - Safe string building in _detect_gpu (no line-continuation backslashes).  
...
def load_style():  
    ...
    progressbar > trough > progress {  
        ...
        background-color: @sol8_progress;  
        border: none;  
        margin: 0;  
        min-height: 40px;  
    }  
    ...
    """  
    sp = Gtk.CssProvider()  
    sp.load_from_data(css.encode("utf-8"))  # encode at runtime  
...
class FFmpegGUI(Gtk.Window):  
    ...
    def __init__(self):  
        ...
        out_box.pack_start(  
            Gtk.Label(label="Output filename (suffix added automatically)", xalign=0),  
            False,  
            False,  
            0,  
        )  
        ...
        col_container.pack_start(  
            Gtk.Label(label="Container (mp4, mkv, mov, avi, webm).", xalign=0),  
            False,  
            False,  
            0,  
        )  
        ...
        col_encoder.pack_start(  
            Gtk.Label(label="Video encoder", xalign=0), False, False, 0  
        )  
        ...
        col_rc.pack_start(  
            Gtk.Label(label="Rate control (NVENC): VBR or CBR.", xalign=0),  
            False,  
            False,  
            0,  
        )  
        ...
        self.cq_label = Gtk.Label(  
            label="Quality (CRF/CQ 0–51; lower = higher quality)", xalign=0  
        )  
        cq_box.pack_start(self.cq_label, False, False, 0)  
        self.adj_cq = Gtk.Adjustment(value=23, lower=0, upper=51, step_increment=1)  
        self.scale_cq = Gtk.Scale(  
            orientation=Gtk.Orientation.HORIZONTAL, adjustment=self.adj_cq  
        )  
        ...
        bitrate_box.pack_start(  
            Gtk.Label(  
                label="Bitrate (kbps) - set 0 for quality-based",  
                xalign=0,  
            ),  
            False,  
            False,  
        ...
        audio_codec_box.pack_start(  
            Gtk.Label(label="Audio encoder", xalign=0), False, False, 0  
        )  
        ...
        info = (  
            "Convert = use the settings above.  Super Convert = GPU‑first (NVENC) for ≥25 fps; "  
    ...
    def on_codec_changed(self, combo):  
        ...
        if is_x26x:  
            self.cq_label.set_text(  
                "Quality (CRF 0–51, x264/x265; lower = higher quality)"  
            )  
        ...
        if dialog.run() == Gtk.ResponseType.OK:  
            ...
            for folder in folders:  
                if self.is_videots(folder):  
                    video_ts_path = (  
                        folder  
                        if os.path.basename(folder).upper() == "VIDEO_TS"  
                        else os.path.join(folder, "VIDEO_TS")  
                    )  
                    if os.path.isdir(video_ts_path):  
                        files = os.listdir(video_ts_path)  
                        self.append_log(  
                            f"VIDEO_TS folder found: {video_ts_path}\nContents: {files}\n"  
                        )  
                    else:  
                        self.append_log(  
                            f"Warning: Expected VIDEO_TS folder inside {folder} not found\n"  
                        )  
                else:  
                    self.append_log(  
                        f"Selected folder {folder} is not a VIDEO_TS folder\n"  
                    )  
    ...
    def on_convert(self, widget):  
        ...
        self.label_eta.set_text(  
            "Overall ETA: calculating…" if self.total_content_seconds > 0 else "Overall ETA: unknown"  
        )  
    ...
    def on_donate_clicked(self, widget):  
        webbrowser.open(  
            "https://www.paypal.com/donate?business=rompstar@gmail.com&amount=5.00"  
        )  
        ...
        try:  
            if self.is_videots(path):  
                ts_path = (  
                    path  
                    if os.path.basename(path).upper() == "VIDEO_TS"  
                    else os.path.join(path, "VIDEO_TS")  
                )  
    ...
    def _bit_depth_from_pix_fmt(self, fmt):  
        fmt = (fmt or "").lower()  
        if any(x in fmt for x in ["p16", "16le", "16be"]):  
            return 16  
        if any(x in fmt for x in ["p14", "14le", "14be"]):  
            return 14  
        if any(x in fmt for x in ["p12", "12le", "12be"]):  
            return 12  
        if any(x in fmt for x in ["p010", "p10", "10le", "10be"]):  
            return 10  
        return 8  
        ...
        try:  
            j = subprocess.check_output(  
                [  
                    ...
                    "v:0",  
                    "-show_entries",  
                    "stream=bits_per_raw_sample,pix_fmt",  
                    "-of",  
                    "json",  
                    path,  
                ],  
        ...
        try:  
            j = subprocess.check_output(  
                [  
                    ...
                    "v:0",  
                    "-show_entries",  
                    "stream=codec_name,bit_rate,width,height,pix_fmt,avg_frame_rate",  
                    "-of",  
                    "json",  
                    path,  
                ],  
    ...
    def _detect_gpu(self, sync_label_update=False):  
        ...
        try:  
            ...
            if "h264_cuvid" in dec:  
                self._nvdec_codecs.add("h264_cuvid")  
            if "hevc_cuvid" in dec:  
                self._nvdec_codecs.add("hevc_cuvid")  
        except Exception:  
            self._nvdec_codecs = set()  
        try:  
        ...
        try:  
            ...
            self._have_cuda_hwaccel = any(  
                line.strip().lower() == "cuda" for line in out.splitlines()  
            )  
        except Exception:  
            self._have_cuda_hwaccel = False  
        ...
        for enc, caps in self._nvenc_caps.items():  
            flags = []  
            if caps.get("temporal_aq"):  
                flags.append("tempAQ")  
            if caps.get("rc_lookahead"):  
        ...
        # Build info lines without backslashes  
        nvdec_line = (  
            f"NVENC: {nvenc_label}"  
    ...
    def _auto_select_best_codec(self):  
        ...
        if target:  
            ...
            if it:  
                ...
                self.append_log(  
                    f"Auto-selected {target} (NVENC) based on detected hardware.\n"  
                )  
    ...
    def build_video_args(  
        ...
        if codec in ("h264_nvenc", "hevc_nvenc"):  
            ...
            if using_cuda_frames:  
                if (  
                    codec == "hevc_nvenc"  
                    and desired_bit_depth >= 10  
                    and caps.get("p010", False)  
                    and not self._legacy_nvenc  
                ):  
    ...
    def choose_super_settings(self, info):  
        ...
        if use_hevc_nvenc or use_h264_nvenc:  
            ...
            if h >= 2160:  
                cq, preset = (20 if codec == "hevc_nvenc" else 18), "slow"  
            elif h >= 1440:  
                cq, preset = (21 if codec == "hevc_nvenc" else 19), "slow"  
            elif h >= 1080:  
                cq, preset = (22 if codec == "hevc_nvenc" else 20), "slow"  
            elif h >= 720:  
                cq, preset = (24 if codec == "hevc_nvenc" else 22), "medium"  
            else:  
                cq, preset = (26 if codec == "hevc_nvenc" else 24), "medium"  
            want_p010 = (  
                (not self._legacy_nvenc)  
        ...
        if h >= 2000:  
            preset, crf, ultra = "fast", 24, True  
        elif h >= 1000:  
            preset, crf, ultra = "medium", 25, False  
        elif h >= 700:  
            preset, crf, ultra = "medium", 27, False  
        else:  
            preset, crf, ultra = "fast", 29, False  
    ...
    def update_overall_eta(self, elapsed_in_current, speed):  
        if self.total_content_seconds > 0:  
            processed = self.completed_content_seconds + min(  
                elapsed_in_current, self.current_item_duration  
            )  
            remaining_content = max(self.total_content_seconds - processed, 0.0)  
            eff_speed = speed if speed and speed > 0 else (  
                self.last_speed if self.last_speed > 0 else 1.0  
            )  
            ...
            part = (  
                min(1.0, elapsed_in_current / self.current_item_duration)  
                if self.current_item_duration > 0  
                else 0.0  
            )  
            overall_frac = (  
                ((done_units + part) / self.total_items) if self.total_items else 0.0  
            )  
    ...
    def process_batch(self):  
        ...
        for idx, path in enumerate(self.batch_files):  
            ...
            self.current_item_duration = (  
                self.item_durations[idx]  
                if idx < len(self.item_durations)  
                else self.compute_item_duration(path)  
            )  
            ...
            if not bitrate_txt.isdigit():  
                GLib.idle_add(  
                    self.label_selected.set_text,  
                    "Bitrate must be numeric (kbps, e.g., 3000).",  
                )  
                break  
            ...
            out_file = (  
                os.path.join(os.path.dirname(work_path), out_file_name)  
                if self.is_videots(work_path)  
                else out_file_name  
            )  
            movflags_args = (  
                ["-movflags", "+faststart"]  
                if ext in ("mp4", "mov", "m4a", "3gp", "3g2", "mj2", "m4v")  
                else []  
            )  
            tag_args = (  
                ["-tag:v", "hvc1"]  
                if (ext in ("mp4", "mov", "m4v") and codec in ("libx265", "hevc_nvenc"))  
                else []  
            )  
            ...
            if self.is_videots(work_path):  
                ...
                if not vobs:  
                    GLib.idle_add(  
                        self.label_selected.set_text, f"No VOB files found in {ts_path}"  
                    )  
                ...
                ret, stderr_text = self.run_ffmpeg_sync(  
                    cmd, vobs[0], duration_override=self.current_item_duration  
                )  
                success = ret == 0  
                ...
                can_hw_frames = (  
                    ...
                    and self._have_cuda_hwaccel  
                    and self._have_hwupload_cuda  
                    and (vf is None)  
                )  
                input_prefix = []  
                forced_decoder = None  
                ...
                video_args = self.build_video_args(  
                    ...
                    self.chk_spatial.get_active(),  
                    ext,  
                    using_cuda_frames=can_hw_frames,  
                    nvenc_safe=False,  
                )  
                ...
                ret, stderr_text = self.run_ffmpeg_sync(  
                    cmd, original_path, duration_override=self.current_item_duration  
                )  
                success = ret == 0  
                ...
                if (  
                    ...
                    and can_hw_frames  
                    and (  
                        "auto_scale_0" in stderr_text  
                        or "Function not implemented" in stderr_text  
                        or "Impossible to convert between the formats" in stderr_text  
                    )  
                ):  
                    self.append_log(  
                        "Retrying NVENC without CUDA hw decode (CPU decode → NVENC)…\n"  
                    )  
                    video_args = self.build_video_args(  
                        codec,  
                        preset,  
                ...
                if (  
                    ...
                    and (  
                        "Temporal AQ not supported" in stderr_text  
                        or "rc-lookahead" in stderr_text  
                        or "Provided device doesn't support required NVENC features" in stderr_text  
                        or "Could not open encoder" in stderr_text  
                    )  
                ):  
                    self.append_log(  
                        "Retrying NVENC in safe mode (no temporal AQ / lookahead)…\n"  
                    )  
                    video_args = self.build_video_args(  
                        codec,  
                        preset,  
                ...
                if not success and codec in ("h264_nvenc", "hevc_nvenc"):  
                    self.append_log(  
                        "Falling back to CPU x264 veryfast (should exceed 25 fps at 1080p)…\n"  
                    )  
    ...
    def run_ffmpeg_sync(self, cmd, input_file, duration_override=None):  
        ...
        self.duration_seconds = (  
            duration_override if (duration_override and duration_override > 0) else self.get_duration(input_file)  
        )  
    ...
    def update_batch_status(self, index, total, path):  
        self.label_batch_status.set_text(  
            f"File ({index} of {total}): {os.path.basename(path)}"  
        )  
        return False  
    ...
    def update_progress(self, fraction):  
        ...
        if self.total_items > 1:  
            self.progressbar.set_text(  
                f"{int(fraction * 100)}% — File {self.current_index}/{self.total_items}"  
            )  
        else:  
