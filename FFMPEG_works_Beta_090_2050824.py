...
class FFmpegGUI(Gtk.Window):
    ...
    def __init__(self):
        ...
        rc_label = Gtk.Label(label="Rate Control - choose the encode video bitrate.  VBR (VAriable Bitrate, dynamic adjusted), CBR (Constant Bitrate, steady bitrate).", xalign=0)
        ...
        cq_label = Gtk.Label(label="Constant Quality - a rate control mode for quality level (CQ value, typically 0-51), and the encoder tries to maintain that consistent visual quality throughout the video. ", xalign=0)
        cq_box.pack_start(cq_label, False, False, 0)

        self.adj_cq = Gtk.Adjustment(value=23, lower=0, upper=51, step_increment=1)
        self.scale_cq = Gtk.Scale(orientation=Gtk.Orientation.HORIZONTAL,
                                  adjustment=self.adj_cq)
        ...
        bitrate_label = Gtk.Label(label="Bitrate (kbps) - Controls data per second. Higher bitrate = better quality and larger file size.", xalign=0)
        ...
        self.chk_spatial = Gtk.CheckButton(label="Enable Spatial Adaptive Quantization - Improves perceptual quality.")
        ...
        audio_codec_label = Gtk.Label(label="Audio Codec - select the audio compression format for the output.", xalign=0)
            ...
            try:
                ...
                while True:
                    ...
                    if m and self.duration_seconds:
                        ...
                        progress = min(elapsed / self.duration_seconds, 1)
