...
class FFmpegGUI(Gtk.Window):  
    ...
    def process_batch(self):  
        ...
        for idx, path in enumerate(self.batch_files):  
            ...
            movflags_args = ["-movflags", "+faststart"] if ext in ("mp4", "mov", "m4a", "3gp", "3g2", "mj2") else []  
            ...
            if self.is_videots(work_path):  
                ...
                # VOBs are effectively 8-bit; still probe first VOB just in case  
                bit_depth = self.get_input_bit_depth(vobs[0]) if vobs else 8  
    ...
    def _detect_gpu(self):  
        ...
        try:  
            ...
            nvenc_label = ", ".join(nvenc) if nvenc else "none"  
        except Exception:  
            self._nvenc_available = set()  
