...
class FFmpegGUI(Gtk.Window):  
    ...
    def __init__(self):  
        ...
        preset_label = Gtk.Label(  
            label="Preset (speed/quality trade‑off; supported by x264/x265 and NVENC).",  
            xalign=0  
        )  
        ...
        self.cq_label = Gtk.Label(  
            label="Quality (CRF 0–51, x264/x265; lower = higher quality)",  
            xalign=0  
        )  
    ...
    def on_codec_changed(self, combo):  
        ...
        if is_x26x:  
            ...
            self.cq_label.set_text("Quality (NVENC CQ 0–51; used with VBR/CQP)")  
            self.scale_cq.set_sensitive(True)  
        else:  
    ...
    def process_batch(self):  
        ...
        for idx, path in enumerate(self.batch_files):  
            ...
            if self.is_videots(work_path):  
                ...
                # Only pass preset when encoder supports it  
                if codec in ["libx264", "libx265", "h264_nvenc", "hevc_nvenc"]:  
                    cmd.extend(["-preset", preset_to_use])  
                # Spatial AQ for NVENC when requested  
                if codec in ["hevc_nvenc", "h264_nvenc"] and self.chk_spatial.get_active():  
